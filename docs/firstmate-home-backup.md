...
If `SNAPSHOT` records the requested home as `uncaptured-home`, the plan opens with a prominent `!! WARNING` naming the recorded reason, and `--apply` repeats it on stderr as its last output.
That means the last backup could not reach that home, so what is about to be placed is the last **successful** capture and is of unknown age - nothing this tool writes carries a timestamp.
It is a warning and never a refusal, and needs no extra flag: during a real outage stale memory beats no memory, and that trade is the operator's to make.
A skipped home is never wiped from the backup repo, which is why the older capture is still there to restore.
