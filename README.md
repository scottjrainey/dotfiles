...
gh-dash (installed as a `gh` CLI extension per bootstrap.sh Step 7, not via Homebrew/Nix) is configured by `home/.config/gh-dash/config.yml`. It adds one custom PR-view keybinding, `i`, that runs [tuicr](https://tuicr.dev/) (`brew install tuicr`, a vim-keybinding code-review TUI) against the currently selected PR via `tuicr pr {{.PrNumber}} --repo-url https://github.com/{{.RepoName}}` - no local clone or `repoPaths` entry needed. See the comments in that file before picking another custom key: gh-dash's own defaults already claim every other single-letter mnemonic on both the global and PR-view keymaps.
